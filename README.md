...
```
┌────────────┐     ┌───────────────────┐     ┌──────────────────┐
│ 20-min 调度 │ ──▶ │  monitor skill    │ ──▶ │  飞书卡片 / 文档   │
└────────────┘     │  （机器驱动）      │     │  飞书多维表格      │
                   └───────────────────┘     └──────────────────┘
                            ▲
┌────────────┐     ┌───────────────────┐
...
Agent 会引导你依次填写 5 项配置，全部完成后自动推送欢迎卡片，并在下一个 20 分钟整点启动监控。
...
系统全程通过**自然语言对话**操作，无需 CLI 命令。下面给出典型场景。
...
下表列出 `agent` skill 识别的用户意图及映射的内部操作（大部分情况用户无需关心）。
...
| 帮助 / help / 怎么用 | `references/help.md` | — |
...
每 20 分钟固定执行 `monitor` skill 的 8 步链条：
